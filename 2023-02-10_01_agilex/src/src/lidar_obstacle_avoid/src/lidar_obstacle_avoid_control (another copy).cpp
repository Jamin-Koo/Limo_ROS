...
void scanCallback(const sensor_msgs::LaserScan::ConstPtr& scan)
{
   ...
   // tunnel_angle.data = lidar_steer;
	...
	if(front_data < 55 && front_data > 15)
	{
		pass_data.data = 1;
		printf("detect!\n");
		printf("obs: %d , pass : %d \n",obs_data.data,pass_data.data);
	...
	//////average_right/////
	if(average_right < 30 && average_right > 10)
	{
		obs_data.data = 1;
	    printf("detect!\n");
	    printf("average_right %0.4f \n", average_right);
	    printf("obs: %d ",obs_data.data);
