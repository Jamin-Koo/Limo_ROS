...
void lane_detection(void)
{
   ...
   float c_x_sum=0;	     
	...
	else if( (left_line_detect_flag != 0) && (right_line_detect_flag == 0))
    {
	  ...
	  lane_center_x = ((l_line_center + (lane_distance)/2) -img_width/2); // 180
	  steer_angle_new2 = error_lane *Kp + Kd*error_lane_delta/1.42857; //1.4
 	  steer_angle_new = RAD2DEG(steer_angle_new2);
	  speed = 0.271;
	...
	else if( (left_line_detect_flag == 0) && (right_line_detect_flag != 0))
    {
	  ...
	  lane_center_x = ((-(lane_distance)/2 + r_line_center) -img_width/2); //130
	  steer_angle_new2 = (error_lane *Kp + Kd*error_lane_delta/1.42857);
	  steer_angle_new = RAD2DEG(steer_angle_new2);
