...
int main(int argc, char **argv)
{
	...
	while(ros::ok())
  {
		    ...
		    if(lidar_flag == 0){
			 ...
			 else
			 {
				if(second_odom_flag == 0)
				{
					...
					pid_control = control_yaw(129.0+30.0);//
				...
				else if(second_odom_flag == 1)
				{
					...
					pid_control = control_yaw(129.0-36.5);
					if(odom_distance > 1.31)
					{
						cmd_vel_msg.linear.x = 0.0;
						cmd_vel_msg.angular.z = 0.0;
						second_odom_flag = 2;
	...
	if(thirth_odom_flag == 0){
		...
		if(stop_line_data == 1){
			pid_control = control_yaw(-140); // +10
			cmd_vel_msg.linear.x = 0.0;
			cmd_vel_msg.angular.z = 0.0;
