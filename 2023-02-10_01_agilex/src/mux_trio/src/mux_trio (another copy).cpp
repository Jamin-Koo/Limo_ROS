	...
	while(ros::ok())
  {
	  ...
	  switch(region_no) {
		...
		break;
		  ...
		  case 1:{
			 pid_control = control_yaw(-92); // +9
			 if(crosswalk_flag == 0){
			    cmd_vel_msg.linear.x = 0.12;
			   	cmd_vel_msg.angular.z = angular_z;
			   	if(angular_z < 0.2 && -0.2 < angular_z){
					angular_z = 0;
					}
		    ...
		    if(lidar_flag == 0){
			 ...
			 else
			 {
				if(odom_flag == 0)
				{
					...
					pid_control = control_yaw(11.0+30.0);//
					if(odom_distance > 0.55) //
					{
						cmd_vel_msg.linear.x = 0.0;
						cmd_vel_msg.angular.z = 0.0;
						odom_flag = 1;
				...
				{
					ROS_INFO("odom_flag_1");
					pid_control = control_yaw(11.0);//--148
					if(odom_distance > 0.84)
					{
						cmd_vel_msg.linear.x = 0.0;
						cmd_vel_msg.angular.z = 0.0;
						odom_flag = 2;
				...
				else if(odom_flag == 1)
				{
					...
					pid_control = control_yaw(11.0-35.5);
					if(odom_distance > 1.05)
					{
						cmd_vel_msg.linear.x = 0.0;
						cmd_vel_msg.angular.z = 0.0;
						odom_flag = 2;
		  ...
		  {  
			  base_line_xy();
			  if(traffic_detect == 0 && traffic_flag == 0){ // red detected
				  base_line_xy();
				  happy_flag = 0;  
				  traffic_flag = 1;
				  }
			  else if(traffic_detect == 1 && traffic_flag == 0){ // green detected
				  happy_flag = 1; 
		  ...
		  case 3:// cha_dan_gi ^^7
		  {
	  	   ...
	  	   if(sub_obs == 1){
			  ...
			  /*if(lidar_detected == 1 && lidar_flag == 2)
			  {  
				base_line_zero();
		    	ros::Duration(3.0).sleep();  
		    	lidar_flag = 3;
