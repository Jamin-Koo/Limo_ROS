...
void mux_mission_Callback(const std_msgs::Int16 & msg)
{
	mux_mission_flag = msg.data;
}
...
void scanCallback(const sensor_msgs::LaserScan::ConstPtr& scan)
{
   ...
   // ROS_INFO("angle_range, %f, %f %f", RAD2DEG(scan->angle_min), RAD2DEG(scan->angle_max), RAD2DEG(scan->angle_increment));
    ...
    {	
        ...
        switch(lidar_flag) {
			case 0:{
				ROS_INFO("case111111111111111\n");
				if((degree>=-3) && (degree<=3))
					{
					  if(scan->ranges[i] <= 0.47 && scan->ranges[i] >= 0.20) 
					  {
						check_case1 = 1;
						cnt_right++;
						lidar_check.data = 1;
					 ...
					 else if(mux_mission_flag == 2)/*(plus >= 50)*/{
							lidar_check.data = 0;
						  }
				...
				if((degree>=-10) && (degree<=10))
						{
						  ...
						  else if(mux_mission_flag == 3)/*(plus >= 50)*/{
							lidar_check.data = 0;
						  } 
						  if(check_case1 == 2 && lidar_check.data == 0){
					      lidar_flag = 2;
					  }
			...
			case 2:{
				//happy_flag == 1
				if(tuner_flag == 0 && lidar_check.data == 0){
				if((degree<=55) && (degree>=10))
				{
				  if(scan->ranges[i] <= 0.7 && scan->ranges[i] >= 0.2 ) 
				  {
					cnt_right++;
				...
				{
				  if(scan->ranges[i] <= 0.9 && scan->ranges[i] >= 0.2) 
				  {
					cnt_left++;
				   // ROS_INFO("cnt_left = %d\n", cnt_left);
				...
				if(average_right_old == average_right){
					tuner_flag = 1;
				}
				...
				average_right_old = average_right;
				break;
			}
...
int main(int argc, char **argv)
{
	...
	std::string mux_mission_topic = "/mux_mission_flag";
	

	ros::param::get("~lidar_topic", lidar_topic);
	ros::param::get("~obs_topic", obs_topic);
	ros::param::get("~mux_mission_topic", mux_mission_topic);
	...
	ros::Subscriber mux_mission = n.subscribe("/mux_mission_flag", 1, mux_mission_Callback);
	...
	//ROS_INFO("average_left = %f\n", average_right);
	//ROS_INFO("average_left_old = %f\n", average_right_old);
	//ROS_INFO("tuner_flag = %d\n", tuner_flag);
	
		obs_pub.publish(obs_data);
		obs_check_pub.publish(lidar_check);
		//ROS_INFO("check_case1 = %d ",check_case1);
		//ROS_INFO("lidar_check = %d ",lidar_check.data);
		ROS_INFO("lidar_flag = %d \n",lidar_flag);
		ROS_INFO("mux_mission_flag = %d \n",mux_mission_flag);
		//ROS_INFO("Plus = %d \n",plus);
